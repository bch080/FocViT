...
import sys
sys.path.insert(0, r'c:\Users\11586\Desktop\fViT')
...
# 计算 FoveatedPatchEmbed 的 token 数量
def compute_foveated_tokens():
    ...
    return len(center_patches), len(outer_patches)

num_center, num_outer = compute_foveated_tokens()
num_patches = num_center + num_outer
print(f"\n【Token 信息】")
print(f"  中心 tokens: {num_center}")
print(f"  外围 tokens: {num_outer}")
...
# ============ 新配置 (B/16 容量) ============
print(f"\n{'='*70}")
print("【配置 1: FoveatedViT-B/8_full (ViT-B/16 容量)】")
print(f"{'='*70}")
...
print(f"  attention_dropout: {config_full.transformer.attention_dropout_rate}")
print(f"  dropout: {config_full.transformer.dropout_rate}")
...
print(f"\n{'='*70}")
print("【FoveatedViT-B/8_full 参数量分解】")
print(f"{'='*70}")
...
# center_proj: 8*8*3 = 192 -> D
center_in = 8 * 8 * 3
center_proj_params = center_in * D + D
print(f"   center_proj: Linear({center_in}, {D})")
print(f"     {center_in} × {D} + {D} = {center_proj_params:,}")

# outer_proj: 8*8*3*4 = 768 -> D
outer_in = 8 * 8 * 3 * 4
outer_proj_params = outer_in * D + D
print(f"   outer_proj: Linear({outer_in}, {D})")
print(f"     {outer_in} × {D} + {D} = {outer_proj_params:,}")

foveated_patch_embed_total = cls_params + center_proj_params + outer_proj_params
print(f"   小计: {foveated_patch_embed_total:,}")
...
# LayerNorm per layer: 2 per LN, 2 LNs per block
ln_params = 2 * D
...
# 总计
total_full = foveated_patch_embed_total + pos_embed_params + encoder_total + head_params
print(f"\n{'='*70}")
print(f"【总计】FoveatedViT-B/8_full: {total_full:,} = {total_full/1e6:.2f}M")
print(f"{'='*70}")

# ============ 旧配置对比 ============
print(f"\n\n{'='*70}")
print("【配置 2: FoveatedViT-Small (原小配置)】")
print(f"{'='*70}")
...
# Patch Embed
cls_s = D_s
center_proj_s = (8*8*3) * D_s + D_s
outer_proj_s = (8*8*3*4) * D_s + D_s
patch_embed_s = cls_s + center_proj_s + outer_proj_s
...
print("【三模型参数量对比】")
print(f"{'='*70}")
print(f"\n  {'模型':<35} {'参数量':>15} {'Token数':>10}")
print(f"  {'-'*35} {'-'*15} {'-'*10}")
print(f"  {'标准 ViT-B/16':<35} {std_total/1e6:>14.2f}M {std_patches+1:>10}")
print(f"  {'FoveatedViT-B/8_full':<35} {total_full/1e6:>14.2f}M {num_patches+1:>10}")
print(f"  {'FoveatedViT-Small':<35} {total_small/1e6:>14.2f}M {num_patches+1:>10}")
print(f"  {'-'*35} {'-'*15} {'-'*10}")
print(f"  {'FoveatedViT-B/8_full vs ViT-B/16 差距':<35} {(total_full-std_total)/1e6:>+14.2f}M")
print(f"  {'相对差距':<35} {(total_full-std_total)/std_total*100:>+14.2f}%")
...
print(f"  FoveatedViT-B/8_full 实际: {actual_full:,} ({actual_full/1e6:.2f}M)")
